...
| `object` (sous-objet), déclaré ou deviné | ✅ | indexé par **chemins pointés** (`client.ville`), comme ES. Un objet n'est pas un champ : il n'existe que par ses feuilles. `GET /_mapping` re-niche les chemins. Un tableau d'objets est aplati — comme ES, la correspondance entre sous-champs d'un même élément est perdue (c'est ce que `nested` corrige). La **forme** du document est vérifiée contre celle du mapping avant tout le reste, exactement là où ES la vérifie — avant `dynamic`, donc `false` et `strict` n'y changent rien : un objet posé sur une feuille rend `failed to parse field [a] of type [keyword] … Preview of field's value: '{b=x}'`, une valeur posée sur un objet rend `object mapping for [a] tried to parse field [a] as object, but found a concrete value`, et une fusion de mapping qui ferait cohabiter `a` et `a.b` rend `can't merge a non object mapping [a] with an object mapping`. Deux différences déclarées avec ES : le préfixe `[ligne:colonne]` qu'il place devant (une position dans le JSON brut, que ferrite n'a plus une fois le corps parsé) et le `caused_by` qu'il joint |
...
| `index` | ✅ | garder un champ **sans le rendre cherchable**. Les deux écritures d'ES sont admises (le booléen et la chaîne), et le rendu du mapping suit le sien : `index: true` ne laisse rien dans `_mapping` (c'est le défaut), `index: false` y est **conservé** — sans quoi un outil qui relit son mapping pour décider s'il doit réindexer y lirait le contraire de ce qu'il a posé.
**Un champ `index: false` reste cherchable s'il a une colonne, et seul son score change.** C'est la première chose à savoir, parce que le nom du paramètre dit le contraire, et parce que c'est ce que rend **Elasticsearch 8.15** — la version que ferrite annonce. Un Elasticsearch **7.10.2** ne fait pas ça : il y refuse la clause (`Cannot search on field [x] since it is not indexed.`) et son `_field_caps` annonce `searchable: false`. Le repli sur les *doc values* est arrivé entre les deux, et la divergence est chiffrée dans [compat-es7.md](compat-es7.md) : **87 cases** sur les 244 de la sonde. ferrite suit la 8.15, comme partout ailleurs.
Ce que le paramètre fait ensuite n'est pas ce que son nom laisse croire, et ça a été **mesuré avant d'être écrit** ([`sonde_index_false.py`](../tests/compat/sonde_index_false.py), 244 questions, contre un ES **8.15.0**) : un champ non indexé garde sa **colonne**, et Elasticsearch y retombe. Sur un `keyword`, un `long`, un `double`, une `date` ou un `boolean`, la recherche continue donc de répondre — `term`, `terms`, `range`, `match`, `match_phrase`, `prefix`, `wildcard`, `regexp`, `fuzzy`, `exists` — et le tri, les agrégations, `fields` et `docvalue_fields` aussi. Le prix est un **balayage** de la colonne au lieu d'un saut par l'index, et un **score constant** de 1.0 (× `boost`) là où un champ indexé est noté par BM25 : une colonne ne porte ni fréquence de terme ni longueur de champ. C'est exactement ce que rend ES.
Sur un `text`, il n'y a pas de colonne (ce serait doubler le stockage du texte) : le champ n'est plus cherchable du tout, et la clause est **refusée** avec la phrase d'ES — `Cannot search on field [x] since it is not indexed.`, et sa variante pour une phrase (`was indexed without position data`). Deux exceptions mesurées : `exists` y rend 200 et **aucun** document, `match_phrase_prefix` aussi. Le surlignage ne rend aucun fragment sur un champ non indexé pour un `term`, un `match` ou un `range` — il part des `Matches` de Lucene, et une lecture de colonne n'en produit pas — mais il en rend un pour la famille des **automates** (`terms`, `prefix`, `wildcard`, `regexp`, `fuzzy`), que Lucene extrait de la requête sans rien demander à l'index, et `no_match_size` s'applique dans tous les cas. Deux bords de plus, mesurés parce qu'ils ne se devinent pas : le refus d'une **phrase** dépend du nombre de termes (un seul, c'est « pas indexé » ; plusieurs, c'est « sans positions » ; zéro, c'est 200 sans document), et sur un `boolean` non indexé un `lt` **efface le reste de l'intervalle** chez ES — `{"gt": true, "lt": false}` y rend les documents à `false`. Ce dernier bord n'existe sur aucun autre type ; ferrite le reproduit, parce que rendre **moins** de documents qu'ES en silence est pire. Supporté : `true` et `false`, en booléen comme en chaîne, sur un champ ou sur un multi-field, `copy_to` depuis **et** vers un champ non indexé (la valeur copiée arrive dans la colonne de la cible, donc elle y est cherchable). Refusé : `index sur un objet ou un nested` (ES le refuse aussi (« has unsupported parameters »)  : un objet n'entre pas dans l'index inverse, seules ses feuilles y entrent), `exists` sur un `text` à la fois `index: false` **et** `store: true` (divergence assumée, et c'est **ES** qui échoue : il y rend un 500 (`FieldExistsQuery requires that the field indexes doc values, norms or vectors`) parce que le champ existe dans son index sans y porter de colonne. Le même champ sans `store` rend 200 et aucun document ; ferrite rend cette réponse-là dans les deux cas. Un 500 ne se reproduit pas), `case_insensitive` sur un `regexp` posé sur un champ non indexé (**ES le refuse lui-même** (`Match flags not yet implemented [256]`) : son automate de `regexp` sur une colonne est construit sans les drapeaux de correspondance. `prefix` et `wildcard`, qui passent par un autre chemin, les gardent), changer `index` sur un champ déjà déclaré (dans les deux sens, comme ES (« Cannot update parameter [index] »)) |
...
| `match_all` | ✅ | `boost` |
| `match_none` | ✅ | |
| `match` | 🟡 | sur un champ non analysé, se comporte comme `term`. Voir [la recherche libre](#la-recherche-libre-multi_match) pour `lenient`. Supporté : `query`, `operator` (`or` / `and`), `boost`, `lenient`, `minimum_should_match` (posé sur le booléen des **positions** analysées, comme chez ES : une chaîne qui ne rend qu'un terme n'a pas de booléen, et le minimum y est alors sans effet (mesuré). C'est là que `query_string` le descend quand toute l'expression tient en un groupe de mots sur un seul champ). Refusé : `fuzziness`, `analyzer`, `zero_terms_query`, `prefix_length`, `auto_generate_synonyms_phrase_query`, `fuzzy_transpositions`, `max_expansions` |
...
| les champs de **métadonnées** en clause (`_id`, `_index`, …) | 🟡 | `{"term": {"_id": "…"}}` rendait `hits.total: 0` **sans erreur** pour un document qui existe : le nom n'était dans aucun mapping, la clause tombait donc dans « champ non mappé », que le défaut d'ES (`allow_unmapped_fields`) transforme en « ne correspond à rien ». Un vide indiscernable d'un document absent, en 200. Ce que fait ES n'est pas « répondre partout » : les 203 questions de [`sonde_meta_champs.py`](../tests/compat/sonde_meta_champs.py) rangent chaque case dans **trois** issues — il répond, il refuse, il rend vide — et ferrite suit les trois. Deux règles mesurées qu'aucune documentation ne donne : `match` sur `_id` **n'analyse pas** la valeur (`"a b"` y rend zéro document), et une clause sur `_index` lit une **expression de nom d'index** (`{"term": {"_index": "logs-*"}}` rend les documents de `logs-2026`), où seule l'étoile est un joker — `?` y est un caractère ordinaire et `\` n'échappe rien. Voir [`src/meta.rs`](../src/meta.rs). Supporté : `_id` (`term`, `terms`, `match`, `match_phrase`, `multi_match` et `exists` répondent, à score **constant** (1.0 × `boost`) ; `prefix`, `wildcard`, `regexp`, `fuzzy`, `match_phrase_prefix` et `range` y sont **refusés avec la phrase d'ES** (« not on [_id] which is of type [_id] »), parce qu'ES les refuse. `fields` rend l'identifiant), `_index` (les mêmes clauses, plus `prefix` et `wildcard` : la valeur est une expression de nom d'index, donc la clause vaut « tous les documents de cet index » ou « aucun ». C'est son seul usage réel — filtrer un index dans une recherche qui en vise plusieurs. Une valeur qui commence par `_` n'est **pas** un nom valide et ES la refuse (`invalid_index_name_exception` / « Invalid index name [_doc], must not start with '_'. ») ; la seule exception est `_all` **exactement**, qui désigne tous les index. Mesuré, et trouvé par le fuzzer : ferrite y rendait vide), `_type` (rend **vide** en 200, comme ES depuis la 8.0 : la parité y est le vide, et en faire une erreur serait une divergence introduite), `_source` (refusé avec les phrases d'ES (« The _source field is not searchable », « does not support match queries »)), `_field_names` (les clauses de terme y rendent vide, `exists` y est refusé par la phrase d'ES (« Cannot run exists query on _field_names »)), `_version` (refusé en clause avec la phrase d'ES (« The _version field is not searchable ») ; `exists` rend tous les documents). Refusé : `_routing` (ferrite est mono-shard et refuse `?routing=` à l'indexation : **aucun** document n'en porte, donc rendre vide serait indiscernable d'un document sans routage. ES y répond en `term`, `terms`, `match`, les motifs, `range` et `exists`), `_seq_no` (le numéro de séquence est tenu (il sert de condition de concurrence) mais n'est pas interrogeable ; ES y répond en `term`, `range`, tri et agrégation. `exists` seul est servi — chaque document en porte un), `_ignored` (ferrite ne tient pas la liste des valeurs qu'un `ignore_above` a écartées ; ES y répond sur toutes les clauses), le **tri**, l'**agrégation** et `docvalue_fields` sur un champ de métadonnées (ES les sert sur `_index`, `_seq_no`, `_version` et `_ignored` (et les refuse sur `_id`, `_source` et `_field_names`) ; ferrite les refuse tous en les nommant. Ce sont les clauses de requête qui sont servies, pas les lectures de colonne) |
...
| `query_string` | 🟡 | le mini-langage du `QueryParser` classique de Lucene, porté règle par règle : ses **états lexicaux** (`^` n'accepte qu'un nombre, et pas même un blanc derrière), son plus-long-match (`AND` est un opérateur, `ANDx` un terme, `and` un terme), et son `addClause` — celui qui décide que `a AND b OR c` n'est ni `(a AND b) OR c` ni `a AND (b OR c)` mais une liste plate où `AND` rend obligatoire la clause **précédente**. Une expression que la grammaire refuse rend `query_shard_exception` avec la phrase d'ES (`Failed to parse query [...]`) ; le détail JavaCC qu'ES range sous `caused_by` n'est pas reproduit. Trois règles mesurées qu' aucune documentation ne donne : une clause dont l'analyzer ne tire **aucun** terme n'est pas « ne correspond à rien », elle **disparaît** de la requête (donc `chat AND …` vaut `chat`, et elle ne compte pas dans le dénominateur d'un `minimum_should_match`) ; `lenient` vaut `true` **par défaut** quand la clause ne vise que `*`, et `false` sinon ; et un motif à jokers est *normalisé* et non analysé — ES n'y applique que les filtres « multi-term aware » (`lowercase`, `asciifolding`, `elision`, `german_normalization`), sauf sous `analyze_wildcard`. Mesuré par [`diff_query_string.py`](../tests/compat/diff_query_string.py). Supporté : `query`, `default_field`, `fields` (avec la pondération `champ^3` et les **motifs** (`t*`, `*`), étendus sur les feuilles du mapping — multi-fields et sous-champs d'objet compris, sous-champs de `nested` exclus (chez ES ils visent les documents cachés et ne rendent jamais rien)), `default_operator`, `analyze_wildcard`, `allow_leading_wildcard`, `lenient` (son défaut se **déduit des champs** : `true` quand la clause ne vise que `*`, `false` dès qu'un champ est nommé (mesuré : `n:abc` rend 200 sans `default_field`, et 400 avec)), `minimum_should_match` (posé sur la racine, et seulement si elle est un booléen — comme chez ES), `boost`, `tie_breaker`, `type` (`best_fields` (le défaut) et `most_fields`, qui vaut exactement un `tie_breaker` de 1.0 (mesuré)), `fuzziness` (`AUTO` ou une distance ; un `~n` explicite hors de `[0, 2]` est refusé avec la phrase d'ES (`Valid edit distances are [0, 1, 2] but was [n]`), et un `~n` fractionnaire avec l'autre (`fuzziness needs to be one of 0.0, 1.0 or 2.0`)), `fuzzy_transpositions`, `time_zone` (porté sur les bornes de date de l'expression), `escape` (la chaîne entière est échappée avant d'être lue, donc plus rien n'y est un opérateur), `auto_generate_synonyms_phrase_query` (accepté dans les deux valeurs : ferrite n'a aucun filtre de synonymes, donc le paramètre ne demande rien), `_name`, la grammaire (`champ:valeur`, `champ:"expression"`, `champ:(a OR b)`, `AND` / `OR` / `NOT`, `&&` / `\|\|` / `!`, `+` / `-`, les parenthèses et leur précédence, les jokers `*` et `?`, le préfixe, la regexp `/motif/`, le flou `~n`, le boost `^n` (sur un terme, une phrase ou un groupe), les bornes `[a TO b]` / `{a TO b}` et leurs formes mixtes, les raccourcis `>`, `>=`, `<`, `<=`, `[* TO *]` (l'existence du champ), `champ:*`, `*:*`, `_exists_:champ`, et l'échappement par `\`). Refusé : la proximité d'une phrase (`"a b"~n`) et `phrase_slop` (c'est le `slop` de `match_phrase`, refusé dans tout ce dépôt : tantivy et Lucene ne comptent pas les déplacements de la même façon au-delà de deux termes), `analyzer` (il remplace l'analyzer du champ pour la requête seule ; ferrite analyse toujours avec le `search_analyzer` du champ visé), `quote_analyzer`, `quote_field_suffix` (il fait chercher une expression entre guillemets dans un **autre** champ que celui nommé, et une phrase posée sur le mauvais champ rend des documents plausibles et faux), `fuzzy_prefix_length`, `fuzzy_max_expansions`, `rewrite`, `fuzzy_rewrite`, `max_determinized_states`, `enable_position_increments` (à `false`, il **rapproche** les termes d'une phrase dès qu'un mot vide a été jeté : c'est un `slop` implicite), `type: cross_fields`, `bool_prefix`, `phrase`, `phrase_prefix` (ils changent la façon dont un terme devient une requête — statistiques fusionnées entre champs, phrase implicite, préfixe du dernier mot), une borne posée sur un champ `text` (`range` y est refusé dans tout ce dépôt) |
| `simple_query_string` | 🟡 | la variante qui **ne lève jamais** : son langage n'a ni champs, ni bornes, ni regexp, et un caractère mal placé y est du texte. `+` est le ET, `\|` le OU, `-` la négation, `"…"` une phrase, un `*` final un préfixe, un `~n` final un flou, et les parenthèses groupent. Quatre règles mesurées : une négation se **compte** (`--chat` en annule une), un opérateur posé avant la première clause ne relie rien (`+chat -chien` rend l'**union**, pas l'intersection), le `~…` court jusqu'au bout du mot et ce qui le suit est lu comme un entier — `~` nu vaut la distance **2** (et non `AUTO`), un nombre est ramené dans `[0, 2]`, et ce qui n'est pas un nombre vaut **0**, donc un terme exact (`chot~x` ne rend aucun document là où `chot~` en rend cinq) —, et les champs s'y combinent en **somme** (un `dis_max` de `tie_breaker` 1.0) là où ceux de `query_string` se combinent au meilleur. Supporté : `query`, `fields` (avec `champ^3` et les motifs, comme `query_string`), `default_operator`, `lenient`, `minimum_should_match`, `boost`, `flags` (les treize drapeaux, combinés par `\|` : ils amputent la grammaire opérateur par opérateur (sans `PREFIX`, une étoile finale redevient un caractère ordinaire ; `NONE` désactive tout). Un nom inconnu rend la phrase d'ES (`Unknown simple_query_string flag [x]`)), `auto_generate_synonyms_phrase_query`, `_name`. Refusé : la proximité d'une phrase (`"a b"~n`) (le même `slop`), `analyzer`, `quote_field_suffix`, `analyze_wildcard` (ES le sert ; il n'est mesuré que sur `query_string`), `fuzzy_prefix_length`, `fuzzy_max_expansions` |
| `intervals`, `terms_set`, `script`, `span_*`… | ❌ | **pas encore** — `parsing_exception: unknown query [...]`, avec la liste des clauses connues |
...
| `post_filter` | ✅ | un filtre appliqué **après** le calcul des agrégations : la mécanique d'une page à facettes, où choisir « rouge » ne fait pas disparaître les autres couleurs du panneau de gauche. Sa place dans la chaîne **est** sa définition, et chaque moitié est mesurée contre ES 8.15 : il filtre les hits, donc `hits.total` et la pagination (un `post_filter` sur deux documents d'un résultat de quatre rend bien `total: 2`) ; il ne filtre **pas** les agrégations, qui rendent les mêmes seaux avec et sans lui ; il ne **note** rien, un `boost: 10` posé dedans ne déplaçant pas le `_score` d'un hit — d'où [`src/postfiltre.rs`](../src/postfiltre.rs), qui rend le score de la sous-requête au bit près là où l'intersection d'un `bool` **sommerait** les scores de ses clauses ; il n'apparaît pas dans l'arbre d'`explain` et ne surligne rien, les deux lisant la requête sans lui ; mais ses clauses nommées sortent quand même dans `matched_queries`, dans la même liste que celles de la requête. Et il filtre aussi les `inner_hits` d'un `collapse`. Mesuré par [`sonde_repli.py`](../tests/compat/sonde_repli.py). Supporté : n'importe quelle clause du Query DSL, y compris nommée par un `_name`, avec `from` / `size`, `sort`, `min_score`, `aggs`, `highlight`, `explain`, `scroll`, `collapse` et le multi-index. Refusé : `null`, une chaîne, une liste (`parsing_exception` / « Unknown key for a VALUE_NULL in [post_filter]. », la phrase d'ES, jeton JSON compris), reproduire le préfixe `[ligne:colonne]` d'un objet vide (ES rend « query malformed, empty clause found at [1:46] » : la position désigne un offset dans le JSON brut, que ferrite n'a plus une fois le corps parsé. Le statut et le refus sont les mêmes) |
| `collapse` | 🟡 | un seul document par valeur de champ — « un résultat par produit, pas un par variante ». Le paramètre a l'air d'un `terms` déguisé et ne l'est pas : il ne regroupe rien, il **choisit un représentant** dans la liste déjà ordonnée. Trois conséquences mesurées contre ES 8.15, et aucune devinable : `hits.total` reste celui d'**avant** repliement (une page qui affiche « 7 résultats » et trois lignes n'est pas en panne) ; `from` et `size` paginent les **groupes**, pas les documents ; et les documents **sans valeur** ne disparaissent pas et ne sortent pas un par un — ils forment **un** groupe, dont le représentant ne porte aucune entrée dans `fields`. Un champ **multivalué** fait tomber la recherche entière, comme chez ES : choisir la première valeur rendrait un repliement plausible et faux, en 200. Les `inner_hits` ne sont pas relus par une seconde recherche — les candidats du groupe sont déjà collectés, ce qui garantit qu'ils voient exactement ce que la page voit, `post_filter` et `min_score` compris. Voir [`src/collapse.rs`](../src/collapse.rs), mesuré par [`sonde_repli.py`](../tests/compat/sonde_repli.py). Supporté : `field` sur un `keyword` ou un numérique, y compris `index: false` (le champ garde sa colonne) et un multi-field (`nom.keyword`), `max_concurrent_group_searches` (accepté et sans effet : chez ES il borne le parallélisme des sous-recherches d'`inner_hits`, et ferrite n'en lance aucune. Il ne peut donc pas changer une réponse — le même raisonnement que `preference`), `inner_hits` : `name` (obligatoire, mesure — la documentation d'ES le présente comme optionnel), `from`, `size` (défaut **3**, pas 10), `sort`, `_source`, et un second niveau de `collapse`, avec `sort` (le représentant est le premier **du tri**), `aggs` (qui ne voient pas le repliement), `post_filter`, `min_score`, `highlight`, `fields`, `docvalue_fields`, `_source` et le multi-index. Refusé : un champ `text`, `date` ou `boolean` (« collapse is not supported for the field [x] of the type [text] », la phrase d'ES — c'est **lui** qui refuse les trois), un champ non mappé (« no mapping found for `x` in order to collapse on », la phrase d'ES), dans un `scroll` (« Validation Failed: 1: cannot use `collapse` in a scroll context; », le type et la phrase d'ES), un sous-champ de `nested` (divergence assumée. ES rend 200 en rangeant **tous** les documents racine dans le groupe sans valeur — ses éléments sont des documents à part, donc la racine n'a pas de colonne. ferrite, lui, en a une à cet endroit et replierait sur les valeurs des éléments : il refuse en le nommant, comme il le fait déjà pour le tri et les agrégations), `highlight`, `docvalue_fields`, `fields`, `stored_fields`, `script_fields`, `runtime_mappings`, `explain`, `version`, `seq_no_primary_term`, `track_scores` dans un `inner_hits` (ES les sert ; les laisser passer rendrait un bloc `inner_hits` amputé en silence), un troisième niveau de repliement (« Invalid token in the inner collapse », la phrase d'ES — il s'arrête à deux niveaux lui aussi), reproduire le numéro de document d'un champ multivalué (divergence assumée. Le refus, son type et son statut sont ceux d'ES (« failed to extract doc:N, the grouping field must be single valued ») ; le numéro est **interne**, celui de Lucene chez ES et celui de tantivy ici, et les deux n'ont aucune raison de coïncider), reproduire la `NullPointerException` d'un `collapse` sans `field` (ES tombe en 500 sur « Cannot invoke "Object.hashCode()" because "pk" is null » : un repliement sans champ n'a pas de clé. ferrite refuse explicitement en 400 — reproduire un plantage de la référence n'aurait aucun intérêt), reproduire le préfixe `[ligne:colonne]` de ses refus de parsing (la même divergence que partout ailleurs : la position désigne un offset dans le JSON brut, que ferrite n'a plus une fois le corps parsé) |
| `search_after` | ✅ | la pagination profonde que la 8.x recommande à la place du `scroll` : le tableau `sort` du dernier hit rendu sert de point de reprise. Elle ne tient qu'à une chose, et ce n'est pas celle que la documentation met en avant : **il faut un tri total**. `search_after` reprend *strictement après* la clé donnée, donc tout document dont la clé est ex æquo avec elle est **sauté** — et ES ne s'en plaint pas (mesuré : `sort: [{"n": "asc"}]`, `search_after: [1]` sur un corpus où trois documents portent `n = 1` rend directement les `n = 2`). C'est pourquoi ES ajoute lui-même `_shard_doc` sous un point-in-time, et pourquoi ferrite l'ajoute aussi. Le point de reprise est converti **au type du champ**, index par index — une date s'y écrit `"2026-01-05"` comme un nombre de millisecondes, là où `missing` n'accepte que le nombre. Mesuré par [`sonde_pagination.py`](../tests/compat/sonde_pagination.py). Supporté : la reprise sur toutes les clés de tri : `keyword`, numérique, `date`, `boolean`, `_score`, `_doc`, `_shard_doc`, `null` comme valeur de reprise (c'est la **sentinelle** d'une valeur absente, donc exactement ce que le tableau `sort` d'un document sans valeur rend sur un `keyword` — un parcours doit pouvoir renvoyer au serveur ce qu'il vient d'en recevoir), la coercition d'ES : `"3"` sur un `long`, un flottant **tronqué vers zéro** (`2,7` et `2,2` reprennent tous deux après 2), un nombre sur un `keyword` (qui devient sa forme texte). Refusé : `from` non nul à côté de `search_after` (comme ES : `action_request_validation_exception`, « [from] parameter must be set to 0 when [search_after] is used »), `search_after` dans un contexte de `scroll` (comme ES ; les deux portent leur curseur à des endroits différents), un tri sans aucun champ (« Sort must contain at least one field. » — et `_score` **n'en est pas un** (mesure), là où `_doc` en est un), une arité qui ne correspond pas au tri (« search_after has N value(s) but sort has M. »), un tableau vide, un non-tableau, un élément qui n'est pas un scalaire (les trois phrases d'ES, coquille comprise (« Values must contains at least one value. »)) |
| `POST /{index}/_pit`, `DELETE /_pit`, et le bloc `pit` du corps | ✅ | le **point-in-time** : une vue figée qu'on interroge autant de fois qu'on veut. Ce n'est pas un `scroll` sous un autre nom — un `scroll` fige un **résultat** (une requête, un classement, un curseur côté serveur), un PIT fige un **lecteur** : il ne connaît aucune requête, et chaque recherche qu'on lui pose est une recherche complète sur l'instantané. C'est ce qui permet de changer de requête, de tri ou de taille de page entre deux appels, et c'est pourquoi le curseur, lui, vit chez le client (`search_after`). ferrite retient par index visé son nom, sa génération et **le `Searcher` tantivy du moment** — mesuré : ouvrir un PIT, écrire un document, puis compter *sous* le PIT rend l'ancien total quand la même recherche hors PIT rend le nouveau. Une recherche sous PIT ne prend **pas** d'index dans l'URL (il vient du PIT), rend `pit_id` en tête de la réponse, et voit son `sort` complété d'un `_shard_doc` implicite dès qu'un tri est demandé. Voir [`src/pit.rs`](../src/pit.rs). Supporté : `keep_alive` (**obligatoire** à l'ouverture (`action_request_validation_exception` sinon, la phrase d'ES) et facultatif dans le bloc `pit` du corps, où il repousse l'échéance. Mêmes durées et même plafond (`1d`) que `scroll`), l'expression d'index habituelle à l'ouverture : nom, liste, motif, alias, `_all`, et `expand_wildcards` / `ignore_unavailable`, `DELETE /_pit` avec `{"id": "..."}` (rend `{"succeeded": true, "num_freed": 1}` en 200, et `{"succeeded": false, "num_freed": 0}` en **404** quand l'identifiant ne désigne plus rien — pas une enveloppe d'erreur, c'est le cas normal d'un client qui nettoie deux fois). Refusé : un index dans l'URL d'une recherche sous PIT (comme ES : deux sources de vérité pour une seule question), `pit` et `?scroll=` ensemble (comme ES), `index_filter` dans le corps de `_pit` (chez ES c'est une **optimisation** (« can match ») qui élague les shards, pas un filtre de résultats ; ferrite n'a qu'un shard par index, donc rien à élaguer — l'accepter en silence laisserait croire qu'un filtre a été posé), `_shard_doc` en clé de tri **hors** d'un point-in-time (comme ES : hors d'une vue figée, un numéro interne change au premier commit, donc une reprise bâtie dessus ne désigne plus rien) |
| `knn`, `seq_no_primary_term`, `suggest`, `rescore`, `track_scores`, `q`, `terminate_after`, `version`, `indices_boost`, `profile`, `slice`, `stats`, `ext`, `retriever` | ❌ | **pas encore** — aucun n'est un obstacle de moteur. `search_after` et `pit`, qui manquaient le plus, sont servis depuis la carte 08 ; `collapse` et `post_filter` depuis la carte 17 |
...
| `score_mode` | 🟡 | le score est celui de la requête interne évaluée à plat, il n'y a pas de score par élément — sauf `none`, qui **est** un score et vaut **zéro** : ES y construit un filtre, pas une jointure notée. ferrite rendait le score du pré-filtre (1.0 sur un `exists`), et ça ne se voyait nulle part, puisqu'un facteur constant ne change pas un ensemble de documents. Sauf sous un `min_score`, qui en fait un seuil : `min_score: 1` y gardait 15 documents là où ES n'en rend aucun, en 200. Trouvé par le fuzzer le jour où `min_score` lui a donné de quoi le voir. Supporté : `none`, `avg`. Refusé : `max`, `min`, `sum` |
