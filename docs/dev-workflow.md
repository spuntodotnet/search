...
| `tests/compat/sonde_vide.py` | le **serveur vide** — 27 questions posées aux deux, dont le corps entier se compare sur un 200 : `_shards` à zéro, `max_score` à `0.0` (et non `null`), pas de section `aggregations`. C'est elle qui a montré qu'ES refuse sans index ce qu'il refuse avec, et seulement ça |
...
| `tests/compat/ponderation.py --rejoue` | ce que **5 312 vraies requêtes** deviennent : la même posée aux deux serveurs sur un index vide, mais dont le mapping est déduit de la requête elle-même (sans quoi toute agrégation serait refusée faute de champ). Ne compare qu'« accepté / refusé » — ce qu'ES refuse aussi sort du dénominateur. C'est lui qui a montré qu'un champ non mappé avalait le refus de `range`, `term`, `terms` et `regexp` |
