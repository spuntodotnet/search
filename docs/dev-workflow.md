...
| `python3 tests/compat/sonde_sous_aggs.py [ferrite] [es]` | Une **sous-agrégation** voit-elle tous les documents de son bucket ? Non, au-delà de ~2 048 documents par segment : les buckets **rares** d'un `terms` ou d'un `range` perdent des documents, en 200, avec un `doc_count` juste. Défaut de tantivy 0.26.1, trouvé par le banc à l'échelle. Refuse de tourner sans ses deux cibles |
...
| `tests/compat/diff_analyzers.py` | les **analyzers** — 38 batteries × 210 textes, comparées sur `(terme, offsets, position)`. C'est elle qui a montré que les offsets d'`_analyze` se comptent en unités UTF-16 et non en octets, et qu'un n-gramme produit par un **filtre** se pose à la position de son mot, pas à la suivante |
...
| `tests/compat/sonde_sous_aggs.py` | ce que le banc à l'échelle a trouvé, **figé** hors d'un corpus de 500 000 documents : la sous-agrégation d'un bucket rare perd des documents au-delà de ~2 048 par segment |
