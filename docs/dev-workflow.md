...
| `python3 tests/compat/sonde_metriques.py [ferrite] [es]` | Compare les trois métriques que ferrite ne délègue pas : `percentiles`, `extended_stats`, `top_hits`. Les trois ne posent pas la même question — la première n'est pas une fonction mais une **structure de données**, et c'est la seule de tout le dépôt dont la décision devait se prendre sur une mesure faite **avant** d'écrire une ligne. `--frontiere` est ce livrable-là : il pose les deux côtés de la bascule d'Elasticsearch (exact jusqu'à 1 999 valeurs par seau, t-digest à partir de 2 000) et imprime l'écart au percentile exact. `--calibrer` rejoue la batterie contre deux Elasticsearch, et le rapport imprime la **version** de chaque cible |
...
| `python3 tests/compat/sonde_explain.py [ferrite] [es]` | **Pourquoi ce document, avec ce score ?** Les trois mécanismes de mise au point — `_name` / `matched_queries`, `explain: true`, la route `GET /{index}/_explain/{id}` — comparés à un vrai Elasticsearch : les noms rendus **et leur ordre** (celui d'une `HashMap` de Java, mesuré contre deux conteneurs), le booléen `matched`, la **forme** de l'arbre et la **valeur** de chacun de ses nœuds. `--ecart` est le vrai livrable : il pose les cinq statistiques du BM25 côte à côte et **recalcule** le score de chaque serveur à partir des siennes — c'est ce qui transforme « les scores diffèrent » en « seuls `N` et `avgdl` diffèrent, et voilà de combien ». `--calibrer` rejoue la batterie contre deux Elasticsearch : elle écrit, donc elle ne peut pas s'étalonner contre un seul |
...
| `python3 tests/compat/diff_query_string.py [ferrite] [es]` | Compare le **mini-langage** de `query_string` et de `simple_query_string` : la grammaire (opérateurs, précédence, parenthèses, bornes, jokers, flou, boost, échappement), ses **bords syntaxiques** (opérateur pendant, guillemet non fermé, `^` sans nombre, borne inachevée) et les vingt-six paramètres des deux clauses. Ce qui est comparé est ce qu'un client lit : les documents rendus **dans leur ordre**, le total, et la **phrase** du refus — celle du `root_cause`, la seule qu'une exception de client officiel remonte. `--calibrer` rejoue la batterie contre deux Elasticsearch, et le rapport imprime la **version** de chaque cible : un étalonnage à deux serveurs de la même version prouve le déterminisme, pas l'indépendance à la version |
...
| `python3 tests/compat/sonde_repli.py [ferrite] [es]` | Compare ce qui fait une **page de résultats** : `collapse` et `post_filter`. Les deux se définissent par leur **place dans la chaîne** plutôt que par ce qu'ils calculent, et c'est donc ça qu'elle mesure — ce qu'ils ne touchent pas (les agrégations, le score, l'arbre d'`explain`, le surlignage) autant que ce qu'ils touchent (`hits.total`, la pagination, `matched_queries`, et les `inner_hits` d'un repliement). Ce qui est comparé est le hit **entier**, chaque bloc `inner_hits` avec son total, son `max_score` et ses documents : un repliement qui rend les bons documents avec le mauvais `total` serait vert sur la liste seule. C'est elle qui a montré que `hits.total` reste celui d'**avant** repliement, que les documents sans valeur font **un** groupe, et qu'un `inner_hits` sur un champ non indexé est refusé alors que le repliement seul y marche. `--calibrer` rejoue la batterie contre deux Elasticsearch ; elle **refuse de tourner** sans ses deux cibles |
...
| `python3 tests/compat/sonde_pagination.py [ferrite] [es] [--parcours N]` | **`search_after`, le point-in-time, et le tri total qu'ils exigent.** La pagination profonde ne tient qu'à une chose, et ce n'est pas celle que la documentation met en avant : `search_after` reprend *strictement après* la clé donnée, donc un tri non total **saute** des documents — et ES ne s'en plaint pas. La sonde compare trois choses que seule la troisième rend utile : les documents rendus dans leur ordre (tableau `sort` compris), le **parcours complet** d'un index page par page (mêmes documents, même ordre, même découpe — un parcours qui rend les bons documents dans le mauvais découpage n'est pas le même parcours), et la **totalité** de `_doc` / `_shard_doc`, qui ne se compare pas à ES mais se vérifie de chaque côté. `--parcours N` fixe la taille du corpus de parcours. `--calibrer` rejoue la batterie contre deux Elasticsearch : elle **écrit** (index, contextes), donc elle ne peut pas s'étalonner contre un seul |
...
| `python3 tests/compat/sonde_meta_champs.py [ferrite] [es]` | Compare les **champs de métadonnées en clause de requête** — `_id`, `_index`, `_routing`, `_seq_no`, `_type`, `_source`, `_field_names`, `_version`, `_ignored` — sur les dix-huit clauses qui prennent un nom de champ. Elle range chaque case dans **trois** issues plutôt que deux, et c'est tout son objet : ES **répond** (`term` sur `_id`), il **refuse** (`prefix` sur `_id`, en nommant le type), ou il rend **vide** (`_type`). Sans la troisième colonne, « faire répondre `_id` » aurait rendu des documents là où ES refuse. `--table` ne demande **qu'un** serveur, n'émet aucun verdict et sert à mesurer une autre version d'ES ; elle imprime la **version de chaque cible**, parce qu'un étalonnage à deux serveurs de la même version ne peut rien dire de la version majeure |
| `python3 tests/compat/sonde_survie.py [ferrite] [es]` | **Le serveur survit-il à la requête ?** La question qui vient avant toutes les autres sondes : un `panic!` n'est pas une erreur 500, le profil de release porte `panic = "abort"` et le processus entier meurt avec tous ses index. Le prédicat est explicite et posé **après chaque cas** — `GET /` doit rendre 200 — et un cas qui le fait tomber est nommé `MORT` plutôt que noyé dans une liste d'écarts. 47 cas : les onze portes du conflit de chemin (un objet posé sur une feuille, une valeur posée sur un objet, une cible de `copy_to`, un `PUT /_mapping`), les sept routes qui lisent une borne de date, et la valeur qui ressemble à un numéro de champ. `FERRITE_CMD` dit comment relancer la cible après un mort. Contre le binaire 0.10.0 : **30 cas MORT**. Voir [`panics.md`](panics.md) |
...
| `tests/compat/diff_aggs.py` | les **agrégations** — 105 requêtes, comparaison du JSON champ par champ, clés comprises (dont 11 sur l'agrégation `filter`, que ferrite exécute lui-même, 8 sur ce qu'un bucket **vide** doit porter, 20 sur `include` / `exclude` et l'ordre par sous-agrégation, et 32 sur les trois métriques de `sonde_metriques.py`) |
| `tests/compat/sonde_metriques.py` | les trois **métriques** que ferrite ne délègue pas — 121 questions posées aux deux serveurs. C'est elle qui a montré qu'Elasticsearch rend le percentile **exact** tant qu'un seau porte moins de 2 000 valeurs (et plus du tout à partir de 2 000 pile), que sa variance est celle de la formule naïve `(Σx² − (Σx)²/n)/n` — donc *moins* juste que celle de tantivy, et c'est la sienne qu'il faut rendre —, et qu'un `top_hits` sans tri porte le score de la requête de la recherche, pas celui du seau |
...
| `tests/compat/sonde_explain.py` | **l'arbre du score** — 54 questions posées aux deux serveurs. C'est elle qui a montré que l'ordre de `matched_queries` est celui d'une table de hachage de Java (donc reproductible, et reproduit), qu'un `bool` dont un `must_not` est un `match_all` **nu** ne nomme rien du tout (ES le réécrit en `match_none` avant même de traduire ses clauses), et — avec `--ecart` — que sur les cinq statistiques du BM25, trois sont identiques entre les deux moteurs et deux ne le sont pas |
...
| `tests/compat/diff_query_string.py` | le **mini-langage** — 687 questions posées aux deux serveurs. C'est elle qui a montré que `lenient` a un défaut **qui se déduit des champs** (`true` quand la clause ne vise que `*`), qu'une clause dont l'analyzer ne tire aucun terme **disparaît** de la requête au lieu de ne rien correspondre, et — avec le fuzzer — que deux mots séparés par un blanc font **une** clause chez ES, ce qui ne se voit que sur un `keyword` |
...
| `tests/compat/sonde_pagination.py` | la **pagination profonde** — 113 questions posées aux deux serveurs, plus six parcours complets comparés page par page. C'est elle qui a montré qu'ES ne se plaint **pas** d'un `search_after` sur un tri non total (il saute les ex æquo, en 200), que trier sur `_id` est **interdit** en 8.x (`Fielddata access on the _id field is disallowed`), et que sous un point-in-time ES **ajoute lui-même** `_shard_doc` au tri — le départage sans lequel `search_after` perdrait des documents |
...
| `tests/compat/sonde_repli.py` | `collapse` et `post_filter` — 97 questions posées aux deux serveurs. C'est elle qui a montré qu'un `post_filter` **filtre aussi les `inner_hits`** d'un repliement, que `inner_hits.name` est obligatoire là où la documentation d'ES le présente comme optionnel, et que sa `size` par défaut vaut **3** et non 10 |
...
| `tests/compat/sonde_meta_champs.py` | les **champs de métadonnées** — 203 questions posées aux deux serveurs. C'est elle qui a montré que `match` sur `_id` **n'analyse pas** sa valeur (`"a b"` y rend zéro document), qu'une clause sur `_index` lit une **expression de nom d'index** où seule l'étoile est un joker, et que `sort: [{"_score": "desc"}]` — le tri par défaut écrit à la main — doit rendre `max_score` renseigné et **aucun** tableau `sort`, comme une recherche sans `sort` |
| `tests/compat/sonde_survie.py` | la **survie** — 47 cas, et après chacun la seule question qui compte d'abord : le serveur répond-il encore ? C'est elle qui a montré qu'un `{"a": {"b": "x"}}` posé sur un mapping où `a` est un `keyword` tuait le processus entier, sans aucun `copy_to`, et que le même défaut avait onze portes d'entrée |
