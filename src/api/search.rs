...
async fn rechercher(
    ...
    if generations.is_empty() {
        ...
        for bloc in repli.iter().flat_map(|d| d.blocs.iter()) {
            if let Some(v) = &bloc.sort {
                // Ni point de reprise ni PIT ici : c'est un `sort` de bloc,
                // pas celui de la page.
                valider_sans_index(None, None, None, Some(v), maintenant, None, false)?;
            }
        }
    }
    ...
    for (nom, uuid, gen, searcher) in generations {
        ...
        // `mut` : les cles de tri d'un `inner_hits` se rangent derriere celles
        // de la racine dans le meme tableau (voir [`crate::collapse`]).
