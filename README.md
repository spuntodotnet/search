...
| RSS au repos | 1,08 Go | **4,9 Mo** |
| Démarrage | 18,1 s | **172 ms** (`docker run` → premier `GET /` servi) |
...
```bash
tar xzf ferrite-v0.11.0-x86_64-unknown-linux-musl.tar.gz && ./ferrite
```
...
| | Elasticsearch 8.15.0 | ferrite 0.11.0 | × |
|---|---|---|---|
| **Image compressée**, telle qu'un registre la sert | 669,1 Mo | **4,3 Mo** | **×155** |
| Image décompressée, ce que son système de fichiers occupe | 1 266,1 Mo | 10,2 Mo | ×124 |
| Le binaire seul | — | 10,2 Mo | |
| Mémoire au repos (RSS) | 1,08 Go | **4,9 Mo** | **×220** |
| Démarrage (`docker run` → premier `GET /` servi) | 18,1 s | **172 ms** (médiane de 5 ; l'essentiel est la création du conteneur par Docker) | ×105 |
...
```
$ ./tests/compat/measure_container.sh ferrite:0.11.0
docker serveur   : 29.7.2 (magasin d'images : overlayfs)

== image : ferrite:0.11.0  (linux/amd64, 1 couche(s))
compressee (registre) :     4 312 537 octets      4,3 Mo   <- ce qu'un `docker pull` telecharge
decompressee (disque) :    10 231 808 octets     10,2 Mo   <- ce que le systeme de fichiers de l'image occupe
binaire /ferrite      :    10 230 016 octets     10,2 Mo   <- le fichier que le conteneur execute

demarrage        : 172 ms (docker run -> premier GET / servi, mediane de 5 tours)
RSS au repos     : 4 808 Ko (4,9 Mo)
```
